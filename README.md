...
Вывести информацию по одному пользователю, VLESS-ссылку и QR-код:

```bash
sudo bash manage-xray-users.sh show user@example.com --server YOUR_SERVER_IP_OR_DOMAIN
sudo bash manage-xray-users.sh show 00000000-0000-4000-8000-000000000000 --server YOUR_SERVER_IP_OR_DOMAIN
```
...
Скрипт перед изменением делает backup рядом с конфигом, проверяет новый JSON через `xray run -test` и перезапускает `xray`. После добавления пользователя или при команде `show` он выводит UUID, VLESS-ссылку и QR-код в терминале, если установлен `qrencode`. Для сухой проверки без перезапуска можно добавить `--no-restart`.
